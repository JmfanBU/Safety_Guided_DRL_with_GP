...
from gpflow.kernels import Sum, Product
...
class Stationary(Kernel):
    ...
    def __init__(self, input_dim, variance=1.0, lengthscales=1.0,
                 noise=None, active_dims=None, ARD=None, name=None):
        """
...
class RBF_regularized(Stationary):
    """
    The radial basis function (RBF) with regularized term
    """

    @params_as_tensors
    def K_r2(self, r2):
        n_dim = tf.shape(r2)
        tr_Lambda = tf.reduce_sum(1./self.lengthscales)
        #Noise
        nconst = tf.exp(-tf.square(self.noise) * tr_Lambda)
        nmat = tf.multiply(nconst, tf.ones(n_dim)) + tf.multiply((1-nconst), tf.eye(n_dim[0], num_columns = n_dim[1]))

        return self.variance * tf.multiply(tf.exp(-r2/2.), nmat)
